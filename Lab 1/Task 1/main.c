...
void key_ini1(void)//инициализаци€ кнопки
{
	...
	GPIO_InitStructure.GPIO_Pin = GPIO_Pin_8;		//перва€ ножка
	GPIO_Init(GPIOA, &GPIO_InitStructure);			//инициализаци€
}
...
int main(void)
{
	...
	{
		if ((GPIO_ReadInputDataBit(GPIOA, GPIO_Pin_8) == 1) && (GPIO_ReadInputDataBit(GPIOC, GPIO_Pin_3) == 1)) //если тумблер on
		{
